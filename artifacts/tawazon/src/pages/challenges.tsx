  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#0d1117', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 28, padding: 32, width: '100%', maxWidth: 420 }}
      >
        ...
        {val === 100 && (
          <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 13, color: '#22c55e', fontWeight: 600 }}>
              {lang === 'ar' ? `🎉 ستكسب ${challenge.xp} نقطة XP عند الحفظ!` : `🎉 You'll earn ${challenge.xp} XP on save!`}
            </span>
          </div>
        )}
    ...
    <div style={{ minHeight: '100vh', padding: '100px 24px 60px', maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      {editingChallenge && (
        <ProgressModal
          ...
          lang={lang}
          onSave={val => setChallengeProgress(editingChallenge.id, val)}
          ...
          {active.length === 0 && completed.length > 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 14 }}>
                {lang === 'ar' ? 'عمل رائع. ترقّب تحديات جديدة قريباً.' : 'Amazing work. New challenges coming soon.'}
              </p>
            </div>
          ) : (
              ...
              {active.map(c => {
                ...
                return (
                  <div key={c.id} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${c.color}28`, borderRadius: 24, padding: 26, position: 'relative', overflow: 'hidden' }}>
                    ...
                    {/* Quick increment buttons + update */}
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        onClick={() => setChallengeProgress(c.id, Math.max(0, prog - 10))}
                      ...
                      <button
                        onClick={() => setChallengeProgress(c.id, Math.min(100, prog + 10))}
        ...
        <div>
          {completed.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <button onClick={() => setTab('active')} style={{ background: '#00f0ff', color: '#03060d', border: 'none', borderRadius: 12, padding: '10px 22px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 14 }}>
                {completed.map(c => (
                  <div key={c.id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 20, padding: 22, display: 'flex', gap: 14, alignItems: 'center' }}>
                    ...
                    {/* Allow resetting */}
                    <button
                      onClick={() => setChallengeProgress(c.id, 0)}
